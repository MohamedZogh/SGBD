...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    ...
    <h1 style="    padding: 80px;">Gestion des Produits :</h1>
<div style="display: flex; justify-content: center">
    <button id="btn-add-product" class="btn btn-info" data-toggle="modal" data-target="#exampleModalCenter2"">Ajouter un produit</button>
</div>
<div id="container-products">
    <div id="products">
        <?php
            foreach ($articles as $article){?>
            <div class="card" data-toggle="modal" data-target="#exampleModalCenter2" data-prix="<?php echo $article['prix'];?>" data-description="<?php echo $article['description'];?>" data-name="<?php echo $article['nom'];?>" data-id="<?php echo $article['id'];?>" style="width: 18rem;">
...
<script>
    $(document).ready(function () {
        ...
        $('.card').click(function(){
            ...
            $('.modal-header button').first().hide();
            $('#putButton').click();
        });
        ...
        $('.gestion').click(function(){
            ...
            gestion == 'Delete' ? ($('.input-gestion').hide(), $('#modal-name').closest('.input-gestion').show()) : $('.input-gestion').show()
            $('#submit-gestion').html(gestion);
        });

        $('#btn-add-product').click( function () {
            $('.modal-header button').hide();
            $('.modal-header button').first().show();
            $('.modal-header button').first().click();
            $('#modal-name').val('');
            $('#modal-id').val('');
            $('#modal-price').val('');
